...
public abstract class Dinosaure extends EntityTameable implements IEntityAdditionalSpawnData
{
	...
	/**
	 * Interpolate the size of the dinosaur based on the day 0 value and the max adult value. If the dinosaur is past Adult age, continue to grow but have diminished growth.
	 */
	...
	/*
	 * public void setPosition(double par1, double par3, double par5) { this.posX = par1; this.posY = par3; this.posZ = par5; float w_2 = this.width / 2.0F * this.HitboxZfactor; float l_2 = this.width
	 * / 2.0F * this.HitboxXfactor; this.boundingBox.setBounds(this.posX - (double)w_2, this.posY - (double)this.yOffset + (double)this.ySize, this.posZ - (double)l_2, this.posX + (double)w_2,
	 * this.posY - (double)this.yOffset + (double)this.ySize + (double)this.height*this.HitboxYfactor, this.posZ + (double)l_2); }
	 * 
	 * protected void setBoundingBox() { float w_2 = this.width / 2.0F * this.HitboxZfactor; float l_2 = this.width / 2.0F * this.HitboxXfactor; this.boundingBox.setBounds(this.posX - (double)w_2,
	 * this.posY - (double)this.yOffset + (double)this.ySize, this.posZ - (double)l_2, this.posX + (double)w_2, this.posY - (double)this.yOffset + (double)this.ySize +
	 * (double)this.height*this.HitboxYfactor, this.posZ + (double)l_2); }
	 */
	...
	public TileEntityFeeder GetNearestFeeder(int SEARCH_RANGE)
	{
		for(int dx = -2; dx != -(SEARCH_RANGE + 1); dx += (dx < 0) ? (dx * -2) : (-(2 * dx + 1))) // Creates the following order: -2,2,-3,3,-4,1,....,10, stops with 10. looks at near places, first
		{
			...
			{
				for(int dz = -2; dz != -(SEARCH_RANGE + 1); dz += (dz < 0) ? (dz * -2) : (-(2 * dz + 1))) // Creates the following order: -2,2,-3,3,-4,1,....,10, stops with 10. looks at near places,
																											// first
				{
	...
	public void readEntityFromNBT(NBTTagCompound var1)
	{
		...
		/*
		 * if (this.getHunger() <= 0) { this.setHunger(this.getMaxHunger()); }
		 */
		...
		else
		{
			...
			if(var2 != null)
			{
				...
				{// Item is one of the dinos food items
					if(!player.worldObj.isRemote)
					{
						...
						{ // The Dino is Hungry and it can eat the item
							...
							--var2.stackSize;
							/*
							 * if (var2.stackSize <= 0) { player.inventory.setInventorySlotContents(player.inventory.currentItem, (ItemStack)null); }
							 */
				...
				// no food, but not nothing
				{
					...
					if(var2.itemID == EGItemList.Whip.itemID && this.isTamed() && this.SelfType.isRideable() && this.isAdult() && !this.worldObj.isRemote && this.riddenByEntity == null && player.username.equalsIgnoreCase(this.getOwnerName()))
					{
			...
			else
			{
				...
				/*
				 * if (this.isTamed() && this.SelfType.isRideable() && this.isAdult() && !this.worldObj.isRemote && (this.riddenByEntity == null || this.riddenByEntity == player)) { player.rotationYaw
				 * = this.rotationYaw; player.mountEntity(this); this.setPathToEntity((PathEntity)null); this.renderYawOffset = this.rotationYaw; return true; }
				 */
			}
	 ...
	 */
	/*
	 * public boolean canBeSteered() { ItemStack var1 = ((EntityPlayer)this.riddenByEntity).getHeldItem(); return var1 != null && var1.itemID == Fossil.whip.itemID; }
	 */
